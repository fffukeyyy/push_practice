...
function handleRenameRequest(params, document) {
    ...
    const currentBlock = (0, ast_2.getBlockAtPosition)(position.line, lines);
    if (!currentBlock) {
        return;
    }
    const isBlockRename = (0, rename_1.isBlockName)(position, currentBlock, lines, document, 'model') ||
        (0, rename_1.isBlockName)(position, currentBlock, lines, document, 'enum') ||
        (0, rename_1.isBlockName)(position, currentBlock, lines, document, 'view') ||
        (0, rename_1.isBlockName)(position, currentBlock, lines, document, 'type');
    ...
    const isEnumValueRename = (0, rename_1.isEnumValue)(currentLine, params.position, currentBlock, document);
    const isValidFieldRename = (0, rename_1.isValidFieldName)(currentLine, params.position, currentBlock, document);
    ...
    if (isBlockRename || isEnumValueRename || isValidFieldRename) {
        const edits = [];
        const currentName = (0, rename_1.extractCurrentName)(currentLine, isBlockRename, isEnumValueRename, isValidFieldRename, document, params.position);
        ...
        if (!isRelationFieldRename &&
            !(0, rename_1.mapExistsAlready)(lineOfDefinition, lines, blockOfDefinition, isBlockRename) &&
            needsMap) {
            // add map attribute
            edits.push((0, rename_1.insertMapAttribute)(currentName, position, blockOfDefinition, isBlockRename));
        }
        // rename references
        if (isBlockRename) {
            ...
            edits.push(...(0, rename_1.renameReferencesForFieldValue)(currentName, params.newName, document, lines, blockOfDefinition, isRelationFieldRename));
        }
        (0, rename_1.printLogMessage)(currentName, params.newName, isBlockRename, isValidFieldRename, isEnumValueRename, currentBlock.type);
        return {
            changes: {
                [document.uri]: edits,
            },
